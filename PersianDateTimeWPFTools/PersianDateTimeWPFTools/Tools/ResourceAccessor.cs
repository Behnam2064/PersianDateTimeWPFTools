...
namespace PersianDateTimeWPFTools.Tools
{
    internal class ResourceAccessor
    {
        ...
        {
            this._controlType = controlType ?? throw new ArgumentNullException(nameof(controlType));
        }
        ...
        public string GetLocalizedStringResource(string resourceName)
        {
            ...
            {
                Assembly assembly = this._controlType.Assembly;
                this._resourceManager = new ResourceManager(assembly.GetName().Name + ".Controls." + this._controlType.Name + ".Strings.Resources", assembly);
            }
            return this._resourceManager.GetString(resourceName);
        }
    }
}
